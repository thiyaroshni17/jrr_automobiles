...
const emptyEntry = () => ({ description: "", amount: "" });
...
export default function PettyCashPage() {
  const { id } = useParams(); // undefined at /pettycash/create
  const isCreate = !id;
  ...
  const [invalidRows, setInvalidRows] = useState([]); // indexes with missing description/amount
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setErr("");
      try {
        const res = await axios.get(backendurl + `/pettycash/${id}`);
        ...
        setForm({
          ...
          entries:
            Array.isArray(d.entries) && d.entries.length
              ...
              : [emptyEntry()],
          totalDailyExpenses: Number(d.totalDailyExpenses || 0)
        });
      } catch (e) {
  ...
  const checkExistingByDate = async (yyyy_mm_dd, opts = { prompt: true }) => {
    ...
    try {
      const url = `${backendurl}/pettycash/list?start=${yyyy_mm_dd}&end=${yyyy_mm_dd}`;
      const res = await axios.get(url);
      const list = Array.isArray(res.data?.data) ? res.data.data : [];
      if (list.length > 0 && list[0]?._id) {
        if (!opts.prompt || window.confirm("Entry for this date already exists.\nOpen it to edit?")) {
          navigate(`/pettycash/${list[0]._id}`, { replace: true });
        } else {
  ...
  // Validation: require date, and for each row, description and amount > 0
  const validate = () => {
    const issues = [];
    ...
    (form.entries || []).forEach((e, i) => {
      const descMissing = !String(e.description || "").trim();
      const amountMissing = !(Number(e.amount) > 0);
      if (descMissing || amountMissing) issues.push(i);
    });
    if (issues.length > 0) {
      setErr("Description and Amount are required for all entries");
  ...
  const toPayload = () => ({
    ...
    // sno auto-filled; totalDailyExpenses recomputed server-side
  });
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (isCreate) {
        const res = await axios.post(backendurl + "/pettycash/create", payload);
        const newId = res?.data?.data?._id;
        setMsg("Created");
        if (newId) navigate(`/pettycash/${newId}`, { replace: true });
      } else {
        await axios.put(backendurl + `/pettycash/update/${id}`, payload);
        setMsg("Saved");
      }
    } catch (e) {
      setErr(
  ...
  const onDelete = async () => {
    if (isCreate) {
      navigate("/pettycashhome");
      return;
    }
    if (!window.confirm("Delete this petty cash day?")) return;
    try {
      await axios.delete(backendurl + `/pettycash/delete/${id}`);
      navigate("/pettycashhome", { replace: true });
    } catch (e) {
      setErr(e?.response?.data?.message || e?.message || "Delete failed");
    }
  };

  // Styling helpers for invalid inputs
  const invalidStyle = (idx, key) =>
    invalidRows.includes(idx) && (key === "description" || key === "amount")
      ? { borderColor: "#ef4444" }
      : {};
  ...
  />
          <div>
            ...
            <div className="subtitle">{isCreate ? "New Petty Cash" : "Edit Petty Cash"}</div>
          </div>
        </div>
        ...
        <div className="row" style={{ marginTop: 12 }}>
          ...
          <table>
              ...
              {(form.entries || []).map((e, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <input
                      value={e.description}
                      onChange={(ev) => onEntryChange(idx, "description", ev.target.value)}
                      placeholder="Description"
                      required
                      style={invalidStyle(idx, "description")}
                    />
                  </td>
                  <td>
                    <input
            ...
            <tfoot>
              <tr>
                <td>Total</td>
                <td colSpan={3}>₹ {computedTotal.toLocaleString("en-IN")}</td>
              </tr>
            </tfoot>
          </table>
